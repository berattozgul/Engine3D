...
public class ObjectLoader {
    ...
    public Model loadOBJModel(String filename) {
        ...
        float[] textureArr = new float[vertices.size() * 2];
        float[] normalArr = new float[vertices.size() * 3];
        for (Vector3i face : faces) {
            processVertex(face.x,face.y,face.y,textures,normals,indices,textureArr,normalArr);
        }
        int[] indicesArr = indices.stream().mapToInt((Integer v) -> v).toArray();
        return loadModel(verticesArr, textureArr, indicesArr);
    }

    private static void processVertex(int pos, int textureCoord, int normal, List<Vector2f> textureCoordList,
                                      List<Vector3f> normalList, List<Integer> indicesList,
                                      float[] textureCordArr, float[] normalArr) {
        indicesList.add(pos);
        if (textureCoord >= 0) {
            Vector2f textureCoordVec = textureCoordList.get(textureCoord);
            textureCordArr[pos * 2] = textureCoordVec.x;
            textureCordArr[pos*2+1]=1-textureCoordVec.y;
    ...
    public int loadTextures(String filename) throws Exception {
        int width, height;
        ByteBuffer buffer;
